...
test('test', async () => {
  ...
  // Click button:has-text("Sign in")
  await Promise.all([
    page.waitForNavigation(/*{ url: 'https://console2.inten.to/dashboard?bucket=1h&endDate=1632474186&group=client&startDate=1632387786' }*/),
  ...
  // Click .jss212
  await page1.click('.jss212');

  // Fill textarea
  await page1.fill('textarea', 'test');

  // Click text=тестTranslated via Yandex Cloud Translate API v2 >> svg
  await page1.click('text=тестTranslated via Yandex Cloud Translate API v2 >> svg');
